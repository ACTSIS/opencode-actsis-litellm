import { budgetUsagePercent } from "./budget.ts";

export { budgetUsagePercent };
...
export function formatThrottleWarning(info: LimitErrorInfo): string {
  ...
  const prefix = limitType
    ? `Rate limit reached ${limitType}`
    : "Rate limit reached";
  if (info.resetsAt) {
...
interface ClassifiedGatewayError {
  kind: LimitErrorInfo["kind"];
  info: LimitErrorInfo;
}

/**
 * Classify a gateway error message for the fetch wrapper to enrich thrown errors.
 * Returns null when the message is not recognized as a gateway budget/limit error.
 */
export function classifyGatewayError(
  message: string | undefined | null,
): ClassifiedGatewayError | null {
  const info = parseLimitError(message);
  if (!info) return null;
  return { kind: info.kind, info };
}
